...
4. **🔍 Widescreen OpenTelemetry Trace Inspector & Total SLA Badge**: An interactive 4-node architecture flowchart (`Frontend ➔ Data Agent Engine ➔ Conversational Analytics Engine (Gemini 2.5) ➔ BigQuery Engine`) with a glowing emerald **Total End-to-End SLA Badge** (`⏱️ Total End-to-End SLA: 36.76s`) and widescreen toggle (`[↔]`). Developers and architects can inspect system instructions, real-time BigQuery job byte billing (e.g. `10.0 MB` billed), and isolate telemetry down to individual turns via an interactive Per-Turn Switcher (`💬 Filter by Turn`).
