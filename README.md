...
* **Free Form Recharts Integration, Wrapping Textarea & Graph Visualizer Spacing**: Replaced single-line Free Form table input with a 2-line wrapping `<textarea>` (`App.tsx`), eliminating horizontal cutoff for long table IDs. Created `/api/sandbox/explore` (`agents.py`) and an interactive `✨ Load Schema & AI Suggestions (↵)` button with Enter key shortcut that queries BigQuery `INFORMATION_SCHEMA.COLUMNS` and uses Gemini 2.5 Flash-Lite to dynamically generate column-aware analytical suggestion cards with shimmer loading animations.
* **Executive Recharts Integration & Interactive Switcher (`VisualizerWidget`)**: Installed `recharts` and upgraded `VisualizerWidget` (`App.tsx`) to render native SVG Recharts featuring glowing vertical gradient fills (`#8b5cf6` to `#3b82f6`), rounded bar corners, and dark glassmorphism tooltips. Added an interactive visualization switcher (`[📊 Bar]`, `[📈 Line]`, `[📉 Area]`) allowing dynamic chart-type morphing on the fly. Upgraded `getChartFields` with a 250-row limit, string numeric parsing (`isNum(val)`), and top-50 row slicing so Free Form mode queries automatically render interactive Recharts visualizations instead of falling back to raw data grids.
* **Spacious Ellipse Layout & Recharts Preview Drawer for Graph Visualizer (`GraphVisualizer.tsx`)**: Replaced tight circular radius node layout with a wide ellipse (`radiusX = 290`, `radiusY = 205`) in `GraphVisualizer` to prevent label and edge arrow collisions in 7-node schemas like The Look Graph. Integrated a `Visual Chart` tab into the `GraphVisualizer` bottom drawer so users can inspect Recharts visualizations directly when clicking table nodes in the schema graph.
* **Streamlined UI & Python Analysis Toggle Cleanup**: Cleaned up the experimental `🐍 Python Analysis Option` checkbox from the sidebar to prevent confusion when the CA API defaults to BigQuery SQL statistical functions like `CORR()`, keeping the interface clean and focused on Data Agents and Free Form Mode.
